...
def connect_two_points(p1x, p1y, p2x, p2y, xgrid, ygrid, mask_arr):
    ...
    b = ((p1y + p2y) - slope * (p1x + p2x)) / 2.0
    x_start = np.min([p1x, p2x])
    x_end = np.max([p1x, p2x])
    for xind in range(
        np.argmin(np.abs(xgrid - x_start)), np.argmin(np.abs(xgrid - x_end)) + 1
    ):
...
def re_rotate(r, z, cm_r, cm_z, angle):
    return (
        cm_r + r * np.cos(-angle) + z * np.sin(-angle),
        cm_z - r * np.sin(-angle) + z * np.cos(-angle),
    )


def max_size(blob_pts_1, blob_pts_2, grid, n_1):
    binsize = 4.0 * (np.max(grid) - np.min(grid)) / n_1
    bins = np.linspace(
        np.min(blob_pts_1),
        np.max(blob_pts_1),
        int((np.max(blob_pts_1) - np.min(blob_pts_1)) / binsize),
    )
    size = 0.0
    for i in range(len(bins) - 1):
        ...
        if len(i_2) == 0:
            continue
        size_i = (np.max(i_2) - np.min(i_2)) / 2.0
        if size_i > size:
            size = size_i
    return size
...
def estimate_blob_stats(args):
    ...
    te_fit_params, te_fit_params_covariance = optimize.curve_fit(
        tanh_fit_func, rho_prof, te_prof, p0=[np.max(te_prof), -10.0, 1.0]
    )
    ne_fit_params, ne_fit_params_covariance = optimize.curve_fit(
        tanh_fit_func, rho_prof, ne_prof, p0=[np.max(ne_prof), -10.0, 1.0]
    )
    ...
    blob_info = {}
    durations, waitings = {}, {}
    durations["value"], durations["rho"], durations["r"], durations["z"] = (
        np.array([]),
        np.array([]),
        np.array([]),
        np.array([]),
    )
    waitings["value"], waitings["rho"], waitings["r"], waitings["z"] = (
        np.array([]),
        np.array([]),
        np.array([]),
        np.array([]),
    )
    last_ending_times = np.zeros((n_x, n_y)) - 1.0

    filenames = sorted(
        glob(
            os.path.join(
                args.path,
                args.shot + "_" + args.time + "_" + "*_" + args.model + ".pickle",
            )
        )
    )
    for seg, filename in enumerate(filenames):
        ...
        for t in times:
            if output_tracking[t]:
                ...
                for tracking in output_tracking[t]:
                    ...
                    cm_r = (cx / n_x) * (
                        np.max(r_highres) - np.min(r_highres)
                    ) + np.min(r_highres)
                    cm_z = (cy / n_y) * (
                        np.max(z_highres) - np.min(z_highres)
                    ) + np.min(z_highres)
                    ...
                    r_grid_rot = (r_grid_frame - cm_r) * np.cos(angle) + (
                        z_grid_frame - cm_z
                    ) * np.sin(angle)
                    z_grid_rot = -(r_grid_frame - cm_r) * np.sin(angle) + (
                        z_grid_frame - cm_z
                    ) * np.cos(angle)
                        ...
                        for i_poly, poly in enumerate(polygon_pred):
                            ...
                            if mask_i.any():
                                ...
                                r_mins.append(
                                    re_rotate(
                                        np.min(blob_pts_r),
                                        blob_pts_z[np.argmin(blob_pts_r)],
                                        cm_r,
                                        cm_z,
                                        angle,
                                    )
                                ...
                                r_maxs.append(
                                    re_rotate(
                                        np.max(blob_pts_r),
                                        blob_pts_z[np.argmax(blob_pts_r)],
                                        cm_r,
                                        cm_z,
                                        angle,
                                    )
                                ...
                                z_mins.append(
                                    re_rotate(
                                        blob_pts_r[np.argmin(blob_pts_z)],
                                        np.min(blob_pts_z),
                                        cm_r,
                                        cm_z,
                                        angle,
                                ...
                                z_maxs.append(
                                    re_rotate(
                                        blob_pts_r[np.argmax(blob_pts_z)],
                                        np.max(blob_pts_z),
                                        cm_r,
                                        cm_z,
                                        angle,
        ...
        for blob_id in current_ids:
            ...
            for i in range(len(t_traj)):
                ...
                dr = (
                    cm_r_traj_smooth[np.min([i + t_margin, len(t_traj) - 1])]
                    - cm_r_traj_smooth[np.max([i - t_margin, 0])]
                )
                dz = (
                    cm_z_traj_smooth[np.min([i + t_margin, len(t_traj) - 1])]
                    - cm_z_traj_smooth[np.max([i - t_margin, 0])]
                )
                ...
                v_r = (
                    2000000.3692258536
                    * np.cos(np.arctan(dz / dr) - np.arctan(slope))
                    * np.sqrt(dr ** 2 + dz ** 2)
                    / (t_f - t_i)
                )
                v_th = (
                    2000000.3692258536
                    * np.sin(np.arctan(dz / dr) - np.arctan(slope))
                    * np.sqrt(dr ** 2 + dz ** 2)
                    / (t_f - t_i)
                )
                v_r_traj.append(v_r)
                ...
                v_R = (
                    2000000.3692258536
                    * np.cos(np.arctan(dz / dr))
                    * np.sqrt(dr ** 2 + dz ** 2)
                    / (t_f - t_i)
                )
                v_z = (
                    2000000.3692258536
                    * np.sin(np.arctan(dz / dr))
                    * np.sqrt(dr ** 2 + dz ** 2)
                    / (t_f - t_i)
                )
                v_R_traj.append(v_R)
                ...
                blob_center_pixels[
                    np.where(
                        radius ** 2
                        > (r_grid_frame - cm_r_traj_smooth[i]) ** 2
                        + (z_grid_frame - cm_z_traj_smooth[i]) ** 2
                    )
                ] = True
            ...
            pulse_start_time = np.zeros((n_x, n_y)) - 1.0
            pulse_end_time = np.zeros((n_x, n_y)) - 1.0
            pulse_start_time_save = np.zeros((n_x, n_y)) - 1.0
            buffer = 5
            """
            for i in range(len(t_traj)):
                mask = blob_info[blob_id]['mask'][i]
            ...
            waitings_arr = np.zeros((n_x, n_y)) - 1.0
            ...
            for i in range(len(t_traj)):
                ...
                idx_waiting = (
                    (mask_traj[i, :, :])
                    & (last_ending_times != -1.0)
                    & (waitings_arr == -1.0)
                )
                waitings_arr[idx_waiting] = (
                    seg * 1000 + t_traj[i] - last_ending_times[idx_waiting]
                )
                last_ending_times[idx_waiting] = -1.0
                ...
                idx_ending_smallgap = (idx_ending) & (
                    mask_traj[min(i + buffer, len(t_traj) - 1), :, :]
                )
                if len(idx_ending_smallgap) > 0:
                    for j in range(buffer):
                        if i + j < len(t_traj):
                            mask_traj[i + j, :, :][
                                (idx_ending_smallgap) & (~mask_traj[i + j, :, :])
                            ] = True
                ...
                durations_arr[idx_ending] = np.maximum(
                    durations_arr[idx_ending], t_traj[i] - pulse_start_time[idx_ending]
                )
                pulse_start_time[idx_ending] = -1.0

                idx_wait_begin = (~mask_traj[i, :, :]) & (mask_covered)
                pulse_end_time[idx_wait_begin] = seg * 1000 + t_traj[i]
                mask_covered[mask_traj[i, :, :]] = True

            last_ending_times[pulse_end_time != -1.0] = pulse_end_time[
                pulse_end_time != -1.0
            ]
            durations["value"] = np.append(
                durations["value"], durations_arr[durations_arr > buffer]
            )
            durations["rho"] = np.append(
                durations["rho"], rho_tref[durations_arr > buffer]
            )
            durations["r"] = np.append(
                durations["r"], r_grid_frame[durations_arr > buffer]
            )
            durations["z"] = np.append(
                durations["z"], z_grid_frame[durations_arr > buffer]
            )
            waitings["value"] = np.append(
                waitings["value"], waitings_arr[waitings_arr > buffer]
            )
            waitings["rho"] = np.append(
                waitings["rho"], rho_tref[waitings_arr > buffer]
            )
            waitings["r"] = np.append(
                waitings["r"], r_grid_frame[waitings_arr > buffer]
            )
            waitings["z"] = np.append(
                waitings["z"], z_grid_frame[waitings_arr > buffer]
            )
            ...
            for i in range(len(t_traj)):
                ...
                if not np.isnan(v_R_traj_smooth[i]) and not np.isnan(
                    v_z_traj_smooth[i]
                ):
                    angle = np.pi / 2.0 + np.arctan(
                        v_z_traj_smooth[i] / v_R_traj_smooth[i]
                    )
                    r_grid_rot = (r_grid_frame - cm_r) * np.cos(angle) + (
                        z_grid_frame - cm_z
                    ) * np.sin(angle)
                ...
                n_e = tanh_fit_func(
                    rho, ne_fit_params[0], ne_fit_params[1], ne_fit_params[2]
                )
                t_e_eV = tanh_fit_func(
                    rho, te_fit_params[0], te_fit_params[1], te_fit_params[2]
                )
                ...
                t_e_K = t_e_eV * 11604.0
                c_s = np.sqrt(k_b * (gamma * t_e_K) / m_i)
                rho_s = m_i * c_s / (e * b_phi)
                a_hat = (
                    a_b
                    * (r ** (1.0 / 5.0))
                    / ((l_p ** (2.0 / 5.0)) * (rho_s ** (4.0 / 5.0)))
                )
                v_hat = v_r / (
                    c_s * ((2.0 * l_p * (rho_s ** 2) / (r ** 3)) ** (1.0 / 5.0))
                )
...
if __name__ == "__main__":
    ...
    with open(
        "blob_stats_" + args.shot + "_" + args.time + "_" + args.model + ".pickle", "wb"
    ) as handle:
